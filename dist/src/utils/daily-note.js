...
export class DailyNoteUtils {
    ...
    notebookApi;
    blockApi;
    constructor(client, documentApi, notebookApi, blockApi) {
        this.client = client;
     ...
     * @returns 今日笔记的文档 ID
     */
    async getOrCreateDailyNote(notebookId) {
        // 获取笔记本配置
        const notebookConf = await this.notebookApi.getNotebookConf(notebookId);
        // 获取今日笔记保存路径模板
        const dailyNoteSavePath = notebookConf.dailyNoteSavePath ||
        ...
        if (notebookConf.dailyNoteTemplatePath) {
            try {
                const templateIds = await this.documentApi.getDocIdsByPath(notebookId, notebookConf.dailyNoteTemplatePath);
        ...
        const dailyNoteId = await this.documentApi.createDocument(notebookId, dailyNotePath, initialContent);
        return dailyNoteId;
    }
    /**
     * 追加内容到今日笔记
     ...
     * @returns 新创建的块 ID
     */
    async prependToDailyNote(notebookId, content) {
        const dailyNoteId = await this.getOrCreateDailyNote(notebookId);
        // 获取今日笔记的第一个子块
