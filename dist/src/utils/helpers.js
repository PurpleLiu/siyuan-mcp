...
export class SiyuanHelpers {
     ...
     * @returns 增强的块信息
     */
    async getEnhancedBlockInfo(blockId) {
        // 通过 SQL 获取完整块信息
        ...
        if (block.parent_id && block.parent_id !== block.id) {
            const parentResponse = await this.client.request('/api/query/sql', {
        ...
        // 获取子块数量
        const childrenResponse = await this.client.request('/api/query/sql', {
        ...
        // 获取笔记本名称
        const notebookResponse = await this.client.request('/api/notebook/lsNotebooks');
     ...
     */
    async getBlockContext(blockId, context = 2) {
        // 获取目标块
        const targetResponse = await this.client.request('/api/query/sql', {
            stmt: `SELECT * FROM blocks WHERE id='${blockId}'`,
        });
        ...
        // 获取同一父块下的兄弟块
        const siblingsResponse = await this.client.request('/api/query/sql', {
     ...
     * @param docId 文档 ID
     */
    async getDocumentOutline(docId) {
        // 获取所有标题块
        const response = await this.client.request('/api/query/sql', {
     ...
     */
    async getRecentlyUpdatedDocuments(limit = 10, notebookId) {
     ...
     * 创建操作结果对象
     */
    createOperationResult(success, id, operation, resourceType, metadata) {
        return {
            success,
            id,
            operation,
